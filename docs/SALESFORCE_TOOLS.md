...
Self-contained Python kit that materializes a curated AI-agent rule, skill, doc, script, manifest, and config set (~58 files) into any new Salesforce repo. Auto-detects `target-org`, API version, Java home, and PMD binary path and substitutes them in.
...
Self-contained Python kit (the `init.py` bootstrapper itself has no third-party dependencies) that materializes a curated AI-agent rule / skill / doc / script / manifest / config set into any new Salesforce repo. The script auto-detects the target workspace's `target-org` alias, API version, Java home, and PMD binary path, and substitutes those values into the generated files so they work out of the box on macOS, Linux, and Windows.
...
# Writes ~58 files into the current directory and prints a summary.
...
| `docs/`                          | 17                                 | `docs/_templates/` design-doc templates (LLD, questions-and-KT, walkthrough, step-by-step AC test), `docs/diagram-conventions.md` (component-scoped mermaid style those diagrams follow), `docs/org-mirror/` (full-org retrieve runbook plus 3 executable shell scripts), `docs/omnistudio/` guides, and `docs/schema-quickref.md`. Includes a stub `docs/omnistudio/org-conventions.md`. |
...
| `.gitignore`                     | 1                                  | Salesforce/LWC/Node/OS noise plus the paths this workflow generates (`.retrieve-logs/`, `.sf-ops/`, Vlocity build temp, and the kit's own install state). **Append-only merge** — an existing `.gitignore` keeps every entry, ordering, and comment it had; only missing patterns are added. |
| `.initagentrulespy-manifest.json` | 1                                 | Install-tracking marker: kit protocol version, status, managed file paths, and the values substituted, so later runs can spot obsolete files and recover a value whose detection fails on a given machine. Gitignored by the kit's own `.gitignore`. |

`initagentrulespy` never installs, updates, inventories, or removes target
`scripts/`. Provision `schemapy` separately when needed; existing project
scripts remain untouched.

Two behaviours are worth calling out. The `docs/org-mirror/*.sh` scripts land at
mode `755`, and permissions are re-checked across every managed path on each run
(not just files that were written), so a script that lost its executable bit is
repaired. And OS/tooling droppings inside `templates/` — `.DS_Store`,
`Thumbs.db`, `desktop.ini`, `__pycache__/`, `.pytest_cache/`,
`.ipynb_checkpoints/`, `*.pyc` — are skipped by the template walk, so they are
never materialized into a bootstrapped project nor counted as kit files.
...
Options:
  ...
  --replace-merged        Replace .gitignore and .vscode/settings.json with the kit
                          version instead of merging into them. DESTRUCTIVE.
  --replace-seeded        Reset the seed-once scaffolds (org-data-model.mdc,
                          org-conventions.md) to the kit's blank version. DESTRUCTIVE.
...
`templates/` ships with seven `{{...}}` placeholder tokens. `init.py` replaces each of them with a runtime-detected (or CLI-supplied) value. Only text files are substituted — the `docs/org-mirror/*.sh` scripts ship byte-identical, reading their org alias from the environment:

| Placeholder          | Becomes                           | Detection chain                                                                                                          |
| -------------------- | --------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| `{{ORG_ALIAS}}`      | Your `target-org` alias           | `--alias` flag → `.sf/config.json` → `.sfdx/sfdx-config.json` → interactive prompt → sentinel `<TARGET_ORG_ALIAS>`       |
| `{{ORG_NAME}}`       | Human-readable project / org name | `--org-name` CLI flag, then whatever a previous install recorded in the marker (default `CURR ORG`)                      |
| `{{API_VERSION}}`    | Salesforce API version            | `--api-version` flag → `sfdx-project.json` `sourceApiVersion` → previous install → sentinel `API_VERSION_UNSET`          |
